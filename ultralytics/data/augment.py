...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        # Rotation 3D
        M_3D = np.array([[np.cos(np.deg2rad(-a)), -np.sin(np.deg2rad(-a)), 0],
    ...
    def apply_3dkeypoints(self, keypoints_3d, M_3D):
        n3d = keypoints_3d.shape[0]
        if n3d == 0:
            return keypoints_3d

        root_joint_idx = 0
        for i in range(n3d):
            root_pose = keypoints_3d[i, root_joint_idx, :]
            root_pose_matrix, _ = cv2.Rodrigues(root_pose)
            rotated_root_pose_matrix = np.dot(M_3D, root_pose_matrix)
            rotated_root_pose, _ = cv2.Rodrigues(rotated_root_pose_matrix)
            keypoints_3d[i, root_joint_idx, :] = rotated_root_pose.reshape(3)
    ...
    def __call__(self, labels):
        ...
        img, M, scale, M_3D = self.affine_transform(img, border)
...
class RandomFlip:
    ...
    def __init__(self, p=0.5, direction='horizontal', flip_idx=None, flip_3didx=None) -> None:
        """
    ...
    def __call__(self, labels):
        ...
        if self.direction == 'horizontal' and random.random() < self.p:
            ...
            if self.flip_3didx is not None and instances.keypoints_3d is not None:
                instances.keypoints_3d = np.ascontiguousarray(instances.keypoints_3d[:, self.flip_3didx, :])
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose([
        ...
        RandomFlip(direction='horizontal', p=hyp.fliplr, flip_idx=flip_idx, flip_3didx=flip_3didx)])  # transforms
