    ...
    def forward(self, inputs, targets, meta_info):
        ...
        if self.mode == 'train':
            ...
            if self.parts == 'body':
                ...
                # loss['smpl_pose'] = self.param_loss(smpl_pose, targets['smplx_pose'], meta_info['pose_valid']) # computing loss with rotation matrix instead of axis-angle can avoid ambiguity of axis-angle. current: compute loss with axis-angle. should be fixed.
                loss['smpl_pose_6d'] = 2.0 * self.param_loss(smplx_pose_6d, targets['smplx_pose_6d'], meta_info['pose_6d_valid'])
                loss['smpl_shape'] = 2.0 * self.param_loss(shape_param, targets['smplx_shape'], meta_info['smplx_shape_valid'][:,None])
